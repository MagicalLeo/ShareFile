...
echo "-----------------------------------------"
if ! grep -q "share.example.com" /etc/dnsmasq.d/nextcloud-internal.conf 2>/dev/null; then
    echo "address=/share.example.com/10.8.0.1" >> /etc/dnsmasq.d/nextcloud-internal.conf
    echo "address=/admin.share.example.com/10.8.0.1" >> /etc/dnsmasq.d/nextcloud-internal.conf
...
echo "-----------------------------------------"
if [ ! -f /etc/letsencrypt/live/share.example.com/fullchain.pem ]; then
    echo "Getting SSL certificate for share.example.com..."
    certbot certonly --nginx -d share.example.com -d admin.share.example.com --non-interactive --agree-tos
...
echo ""
echo "URLs:"
echo "  Download (Public): https://share.example.com/download/[id]"
echo "  Admin (VPN only):  https://admin.share.example.com/admin"
echo ""
